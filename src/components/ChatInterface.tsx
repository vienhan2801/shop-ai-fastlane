...
export function ChatInterface({ onSelectSuggestion }: ChatInterfaceProps) {
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>(initialMessages);
  const [newMessage, setNewMessage] = useState('');
  const [showSuggestions, setShowSuggestions] = useState(true);
  const [isBotLoading, setIsBotLoading] = useState(false); // Added for bot loading

  const suggestions = [
    { id: 's1', text: 'Sản phẩm thể thao', keyword: 'thể thao' },
    { id: 's2', text: 'Phụ kiện công sở', keyword: 'phụ kiện công sở' },
    { id: 's3', text: 'Thiết bị điện tử', keyword: 'thiết bị điện tử' },
  ];
  ...
  const checkForKeywords = (message: string) => {
    ...
    return message; // Return original message if no keywords found
  };
  ...
  const handleSendMessage = () => {
    ...
    // Check for keywords and apply filter
    const keyword = checkForKeywords(newMessage);
    onSelectSuggestion(keyword);

    // Show suggestions after first user message
    ...
    setTimeout(() => {
      ...
      if (keyword !== newMessage) {
        botMessageContent = `Đã tìm thấy các sản phẩm liên quan đến "${keyword}". Bạn có muốn xem thêm không?`;
      } else {
        botMessageContent = `Đang tìm kiếm sản phẩm phù hợp với "${newMessage}"...`;
      }
  ...
  const handleSuggestionClick = (suggestion: string) => {
    setIsBotLoading(true);
    // Update the filter in the parent Shop component
    ...
    setTimeout(() => {
      const botMessage: ChatMessage = {
        id: (Date.now() + 1).toString(),
        content: `Đã hiển thị các sản phẩm liên quan đến "${suggestion}". Bạn có muốn tìm thêm sản phẩm khác không?`,
        isBot: true,
        timestamp: new Date()
      };
      setChatMessages(prev => [...prev, botMessage]);
    ...
    <div className="w-full h-full border-r border-border flex flex-col bg-card relative">
        ...
        <div className="absolute bottom-32 right-4 flex flex-col items-end gap-2">
          {suggestions.map((suggestion) => (
            <div
              ...
              className="px-2.5 pb-1 pt-0.5 border border-[#0abf86] text-[#0abf86] w-fit text-right text-[16px] leading-[140%] rounded-xl hover:bg-[#0abf86] hover:text-white cursor-pointer transition-colors"
            >
              {suggestion.text}
            </div>
          ))}
      ...
      <div className="p-4 border-t border-border bg-background">
        ...
        <div className='flex items-center justify-center w-full px-4 flex-col text-center space-y-[1px] pt-2 pb-[2px]'>
          ...
          <a href="https://easyaichat.app/" target='_blank' className='text-[10px] text-gray-500 font-medium flex items-center space-x-1 px-2 py-[2px] hover:bg-gray-100 rounded-3xl'>
